# 🐳✨ What is Dockerfile ?
 * Dockerfile is a text file with a set of instructions (e.g., `FROM`, `RUN`, `COPY`, `CMD`) to build a Docker image.
 * It tells Docker how to create an image step by step.
...
```
 * ✅ Best Practice:
    * I prefer using `COPY` for most use cases because it's clearer and more secure.
    * I use ADD only when I need to `extract archives` or work with remote URLs.
